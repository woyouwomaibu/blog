...
import {
  ...
  toDisplayString,
  toHandlerKey
} from "./chunk-IJVEKPGJ.js";
import {
  __esm,
  __export,
  init_define_GLOBAL_PAGES,
  init_define_MZ_ZOOM_OPTIONS,
  init_define_SEARCH_HOT_KEYS,
  init_define_SEARCH_LOCALES
} from "./chunk-XSG5OZUJ.js";
...
function pausableWatch(source, cb, options = {}) {
  ...
  const stop2 = watchWithFilter(source, cb, Object.assign(Object.assign({}, watchOptions), { eventFilter }));
...
var createKeyPredicate = (keyFilter) => typeof keyFilter === "function" ? keyFilter : typeof keyFilter === "string" ? (event) => event.key === keyFilter : keyFilter ? () => true : () => false;
...
function useActiveElement(options = {}) {
  ...
  return computed(() => {
    counter.value;
    return window2 === null || window2 === void 0 ? void 0 : window2.document.activeElement;
  });
}
...
function useBrowserLocation({ window: window2 = defaultWindow } = {}) {
  const buildState = (trigger) => {
    ...
    const { hash, host, hostname, href, origin, pathname, port, protocol, search } = (window2 === null || window2 === void 0 ? void 0 : window2.location) || {};
    return {
      trigger,
      state: state2,
      length,
...
function useCssVar(prop, target, { window: window2 = defaultWindow } = {}) {
  ...
  const elRef = computed(() => {
    var _a;
    return unrefElement(target) || ((_a = window2 === null || window2 === void 0 ? void 0 : window2.document) === null || _a === void 0 ? void 0 : _a.documentElement);
  });
  watch(elRef, (el) => {
    if (el && window2)
...
function useStorage(key, initialValue, storage = defaultWindow === null || defaultWindow === void 0 ? void 0 : defaultWindow.localStorage, options = {}) {
  var _a;
  const { flush = "pre", deep = true, listenToStorageChanges = true, window: window2 = defaultWindow, eventFilter, onError = (e) => {
    console.error(e);
  } } = options;
...
function useDark(options = {}) {
  const { selector = "html", attribute = "class", valueDark = "dark", valueLight = "", window: window2 = defaultWindow, storage = defaultWindow === null || defaultWindow === void 0 ? void 0 : defaultWindow.localStorage, storageKey = "vueuse-color-scheme", listenToStorageChanges = true } = options;
  ...
  const onChanged = options.onChanged || ((v) => {
    const el = window2 === null || window2 === void 0 ? void 0 : window2.document.querySelector(selector);
    if (attribute === "class") {
...
function useRefHistory(source, options = {}) {
  ...
  const { ignoreUpdates, ignorePrevAsyncUpdates, stop: stop2 } = ignorableWatch(source, commit, { deep, flush, eventFilter: composedFilter });
...
function useDebouncedRefHistory(source, options = {}) {
  ...
  const history = useRefHistory(source, Object.assign(Object.assign({}, options), { eventFilter: filter }));
...
function useDevicesList(options = {}) {
  const { navigator = defaultNavigator, requestPermissions = false, onUpdated: onUpdated2 } = options;
  const devices = ref([]);
  ...
  async function update() {
    ...
    onUpdated2 === null || onUpdated2 === void 0 ? void 0 : onUpdated2(devices.value);
  }
...
function useDraggable(target, options = {}) {
  var _a, _b;
  const draggingElement = (_a = options.draggingElement) !== null && _a !== void 0 ? _a : defaultWindow;
  const position = ref((_b = options.initialValue) !== null && _b !== void 0 ? _b : { x: 0, y: 0 });
  ...
  const start = (e) => {
    ...
    if (((_a2 = options.onStart) === null || _a2 === void 0 ? void 0 : _a2.call(options, pos, e)) === false)
      return;
  ...
  const move = (e) => {
    ...
    (_a2 = options.onMove) === null || _a2 === void 0 ? void 0 : _a2.call(options, position.value, e);
    preventDefault(e);
  };
  const end = (e) => {
    if (!filterEvent(e))
  ...
  return Object.assign(Object.assign({}, toRefs2(position)), { position, isDragging: computed(() => !!pressedDelta.value), style: computed(() => `left:${position.value.x}px;top:${position.value.y}px;`) });
}
...
function useResizeObserver(target, callback, options = {}) {
  const { window: window2 = defaultWindow } = options, observerOptions = __rest2(options, ["window"]);
  let observer;
...
function useElementVisibility(element, { window: window2 = defaultWindow, scrollTarget } = {}) {
  ...
  if (window2)
    tryOnMounted(() => useEventListener((scrollTarget === null || scrollTarget === void 0 ? void 0 : scrollTarget.value) || window2, "scroll", testBounding, { capture: false, passive: true }));
...
function useFavicon(newIcon = null, options = {}) {
  ...
  const applyIcon = (icon) => {
    document2 === null || document2 === void 0 ? void 0 : document2.head.querySelectorAll(`link[rel*="${rel}"]`).forEach((el) => el.href = `${baseUrl}${icon}`);
  };
  watch(favicon, (i, o) => {
    if (isString(i) && i !== o)
      applyIcon(i);
...
function useFetch(url, ...args) {
  ...
  const { fetch = defaultWindow === null || defaultWindow === void 0 ? void 0 : defaultWindow.fetch, initialData } = options;
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    return new Promise((resolve, reject) => {
      var _a2;
      fetch(context.url, Object.assign(Object.assign(Object.assign({}, defaultFetchOptions), context.options), { headers: Object.assign(Object.assign({}, headersToObject(defaultFetchOptions.headers)), headersToObject((_a2 = context.options) === null || _a2 === void 0 ? void 0 : _a2.headers)) })).then(async (fetchResponse) => {
...
function useFullscreen(target, options = {}) {
  const { document: document2 = defaultDocument } = options;
  const targetRef = target || (document2 === null || document2 === void 0 ? void 0 : document2.querySelector("html"));
  ...
  async function exit() {
    if (!isSupported)
      return;
    if (document2 === null || document2 === void 0 ? void 0 : document2[ELEMENT])
  ...
  if (document2) {
    useEventListener(document2, EVENT, () => {
      isFullscreen.value = !!(document2 === null || document2 === void 0 ? void 0 : document2[ELEMENT]);
    }, false);
  }
  return {
    isSupported,
...
function useMouse(options = {}) {
  const { touch = true, resetOnTouchEnds = false, initialValue = { x: 0, y: 0 }, window: window2 = defaultWindow } = options;
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = ref(target !== null && target !== void 0 ? target : window2 === null || window2 === void 0 ? void 0 : window2.document.body);
...
function useMutationObserver(target, callback, options = {}) {
  const { window: window2 = defaultWindow } = options, mutationOptions = __rest2(options, ["window"]);
  let observer;
  const isSupported = window2 && "IntersectionObserver" in window2;
  const cleanup = () => {
    if (observer) {
...
function usePointer(options = {}) {
  ...
  return Object.assign(Object.assign({}, toRefs2(state)), { isInside });
}
var SwipeDirection;
...
function useSwipe(target, options = {}) {
  ...
  let listenerOptions;
  const isPassiveEventSupported = checkPassiveEventSupport(window2 === null || window2 === void 0 ? void 0 : window2.document);
  if (!passive)
...
function useScriptTag(src, onLoaded = noop, options = {}) {
  const { immediate = true, manual = false, type = "text/javascript", async = true, crossOrigin, referrerPolicy, noModule, defer, document: document2 = defaultDocument } = options;
...
function useScroll(element, options = {}) {
  const { throttle = 0, idle = 200, onStop = noop, onScroll = noop, eventListenerOptions = {
    capture: false,
    passive: true
  } } = options;
  const x = ref(0);
...
function useTimeAgo(time, options = {}) {
  const { controls: exposeControls = false, max, updateInterval = 3e4, messages = DEFAULT_MESSAGES, fullDateFormatter = DEFAULT_FORMATTER } = options;
  const { abs, round } = Math;
  const _a = useNow({ interval: updateInterval, controls: true }), { now: now2 } = _a, controls = __rest2(_a, ["now"]);
  function getTimeago(from, now3) {
    ...
    if (typeof max === "string") {
      const unitMax = (_a2 = UNITS.find((i) => i.name === max)) === null || _a2 === void 0 ? void 0 : _a2.max;
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = ref((_a = newTitle !== null && newTitle !== void 0 ? newTitle : document2 === null || document2 === void 0 ? void 0 : document2.title) !== null && _a !== void 0 ? _a : null);
  watch(title, (t, o) => {
    if (isString(t) && t !== o && document2)
      document2.title = t;
  }, { immediate: true });
  if (observe && document2) {
    useMutationObserver((_b = document2.head) === null || _b === void 0 ? void 0 : _b.querySelector("title"), () => {
...
function useTransition(source, options = {}) {
  const { delay = 0, disabled = false, duration = 1e3, onFinished = noop, onStarted = noop, transition = identity } = options;
  ...
  const { resume, pause } = useRafFn(() => {
    ...
    outputVector.value = startVector.map((val, i) => {
      var _a;
      return val + ((_a = diffVector[i]) !== null && _a !== void 0 ? _a : 0) * currentTransition.value(progress);
    });
    if (progress >= 1) {
      pause();
      onFinished();
  ...
  const start = () => {
    ...
    diffVector = outputVector.value.map((n, i) => {
      var _a, _b;
      return ((_a = sourceVector.value[i]) !== null && _a !== void 0 ? _a : 0) - ((_b = outputVector.value[i]) !== null && _b !== void 0 ? _b : 0);
    });
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  function write(params2, shouldUpdateParamsMap) {
    ...
    const query = empty ? hashWithoutParams.value : mode === "hash" ? `${hashWithoutParams.value}?${params2}` : `?${params2}${hashWithoutParams.value}`;
    if (window2)
...
function useUserMedia(options = {}) {
  ...
  async function _stop() {
    var _a2;
    (_a2 = stream.value) === null || _a2 === void 0 ? void 0 : _a2.getTracks().forEach((t) => t.stop());
    stream.value = void 0;
  }
  function stop2() {
    _stop();
...
function useWebSocket(url, options = {}) {
  ...
  if (options.heartbeat) {
    const { message = "ping", interval = 1e3 } = resolveNestedOptions(options.heartbeat);
