import { noop, isClient, isString, tryOnScopeDispose, promiseTimeout, increaseWithUnit, useTimeoutFn, watchWithFilter, tryOnMounted, isFunction, timestamp, pausableFilter, ignorableWatch, debounceFilter, createFilterWrapper, bypassFilter, createSingletonPromise, toRefs, containsProp, createEventHook, throttleFilter, isObject, isNumber, useIntervalFn, objectPick, tryOnUnmounted, useDebounceFn, useThrottleFn, identity, clamp, pausableWatch } from '@vueuse/shared';
...
function asyncComputed(evaluationCallback, initialState, optionsOrRef) {
    ...
    const { lazy = false, evaluating = undefined, onError = noop, } = options;
...
function useRefHistory(source, options = {}) {
    ...
    const { ignoreUpdates, ignorePrevAsyncUpdates, stop, } = ignorableWatch(source, commit, { deep, flush, eventFilter: composedFilter });
...
function useDraggable(target, options = {}) {
    ...
    const start = (e) => {
        ...
        if (((_a = options.onStart) === null || _a === void 0 ? void 0 : _a.call(options, pos, e)) === false)
            return;
    ...
    const move = (e) => {
        ...
        (_a = options.onMove) === null || _a === void 0 ? void 0 : _a.call(options, position.value, e);
        preventDefault(e);
    };
    const end = (e) => {
        if (!filterEvent(e))
    ...
    return Object.assign(Object.assign({}, toRefs(position)), { position, isDragging: computed(() => !!pressedDelta.value), style: computed(() => `left:${position.value.x}px;top:${position.value.y}px;`) });
}
...
function useFullscreen(target, options = {}) {
    const { document = defaultDocument } = options;
    const targetRef = target || (document === null || document === void 0 ? void 0 : document.querySelector('html'));
...
function usePointer(options = {}) {
    ...
    return Object.assign(Object.assign({}, toRefs(state)), { isInside });
}
...
function useScroll(element, options = {}) {
    ...
    if (element) {
        ...
        useEventListener(element, 'scroll', throttle
            ? useThrottleFn(onScrollHandler, throttle)
            : onScrollHandler, eventListenerOptions);
    }
    return {
        x,
        y,
